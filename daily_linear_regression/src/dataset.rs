...
impl<B: Backend> DailyLinearBatcher<B> {
    ...
    pub fn denorm_targets(
        &self,
        normalized: Tensor<B, 1>,
        min: &Tensor<B, 1>,
        max: &Tensor<B, 1>,
    ) -> Tensor<B, 1> {
        let de_scaled_norm = normalized / 2.0 + 1.0;
        let denominator = (max.clone() - min.clone()).clamp(1e-8, f32::MAX);

        de_scaled_norm * denominator
    }
...
impl<B: Backend> Batcher<DailyLinearItem, DailyLinearBatch<B>> for DailyLinearBatcher<B> {
    fn batch(&self, items: Vec<DailyLinearItem>) -> DailyLinearBatch<B> {
        ...
        for item in items.iter() {
            ...
            let vwop_distance = item.volume_weighted_price - item.close_price;
        ...
        let targets = items
            .iter()
            .map(|item| Tensor::<B, 1>::from_floats([item.next_period_price], &self.device))
            .collect();
